...
!与相干态et萌卡需要的矩阵dim一样
	...
	subroutine detmcstep(mat,s,n,nt,dt,lattice,det0,wht)
	!func 为需要sample的关联函数
	implicit none
...
program main
	use LFMontecarlo
	implicit none
	integer ,parameter :: nx=8,ny=8,n=nx*ny,ntm=8,ndot=1000
	integer :: lattice(n,3),i,j,m,x0,y0,pp
	real :: phi(nx*ny,ntm),del,k,ms,vol(nx*ny,nx*ny),bn,dett(ndot),wht,wt,bb
	...
	wt=0
	do i=1,ndot
		call detmcstep(mat,phi,n,ntm,del,lattice,dett(i),wht)
		dett(i+1)=dett(i)
		wt=wt+wht
		write(*,*) i,dett(i),wt
		bb=bb+dett(i)
